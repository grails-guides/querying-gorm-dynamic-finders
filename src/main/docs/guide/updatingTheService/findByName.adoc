...
[source,groovy]
----
include::initial/grails-app/services/demo/QueryService.groovy[tag=findByName]
----
...
Every dynamic query starts with a prefix. In this case, since we expect to match only one game
instance (i.e. searching `Game` by its `name` property, which is unique), we can use `findBy`,
which will return either a matching instance or a `null` if no matching instance is found.
...
[source,groovy]
----
include::complete/grails-app/services/demo/QueryService.groovy[tag=findByName]
----
