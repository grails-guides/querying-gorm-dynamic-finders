...
NOTE: To find non-strategy games, the shorter `findAllByStrategy(false)` is also a
solution, but only because the property is a boolean. `NotEqual` will work equally
well for any database types; it is used here with a boolean property simply to
